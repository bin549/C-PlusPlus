...
bool Game::Save(const string &fn)
{
    ...
    bfs << static_cast<char>(SAV2ID3 >> 8) << static_cast<char>(SAV2ID3) << Int2Str(loadver) << loadver << HeaderSAV(conf.CurrentFileInfo(), conf.PriceLoyaltyVersion());

    ByteVectorWriter bfz(226 * 1024);
    bfz.SetBigEndian(true);
    bfz << loadver << World::Get() << Settings::Get() << GameOver::Result::Get() << GameStatic::Data::Get() << MonsterStaticData::Get() << SAV2ID3;
    bfs << bfz.data();
