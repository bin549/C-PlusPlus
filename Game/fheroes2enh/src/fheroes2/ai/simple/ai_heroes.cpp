...
string AI::HeroesString(const Heroes &hero)
{
    ...
    os << "flags           : " << (hero.Modes(HEROES_SCOUTER) ? "SCOUTER," : "") << (hero.Modes(HEROES_HUNTER) ? "HUNTER," : "") << (hero.Modes(HEROES_WAITING) ? "WAITING," : "") << (hero.Modes(HEROES_STUPID) ? "STUPID" : "") << endl;
...
s32 FindUncharteredTerritory(Heroes &hero, uint32_t scoute)
{
    ...
    v.resize(distance(v.begin(),
                      remove_if(v.begin(), v.end(),
                                [](s32 it)
                                { return Maps::TileIsUnderProtection(it); })));
